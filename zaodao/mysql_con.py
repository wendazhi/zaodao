...
class MySQLet:
    ...
    def __query(self, **kwargs):
        ...
        result = [dict(zip(cursor.column_names, row)) for row in rows] if kwargs["whole"] else dict(
...
if __name__ == "__main__":
    mysqlet = MySQLet(host="127.0.0.1", user="root", password="$4jj", charset="utf8", database="test", port=3306)
    ...
    print(mysqlet.findKeySql(Const.UPDATE_BY_ATTR, table="info", data={"name": "-09"}, params={"id": 18, "name": "333"},
                             join='AND'))
